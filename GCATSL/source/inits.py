...
from dataprocessor import generate_global_interaction_matrix        

def load_data(train_arr, test_arr, cv, args, labels):
    features_list = []
    ...
    #features1 = np.loadtxt("D:/anaconda3/work/GCATSL-master/data/feature_ppi_sparse.txt")
    #features2 = np.loadtxt("D:/anaconda3/work/GCATSL-master/data/feature_Human_GOsim_BP.txt")    
    #features3 = np.loadtxt("D:/anaconda3/work/GCATSL-master/data/feature_Human_GOsim_CC.txt")   
    ...
    for i in range(args.n_feature):
        path_feature = os.path.normpath(feature_dir + 'feature_' + str(i+1) + '.txt')
        feature = np.loadtxt(path_feature)
    ...
    #print("Starting to extract global neighbors.")
    ...
    #print("Finishing the extraction of global neighbors.")
    data_path = args.input_dir
    path_global = os.path.normpath(data_path + 'interaction_global_' + str(cv) + '.txt')
    
    if not os.path.exists(path_global):
       print("Global interaction matrix does not exist. Please generate it!") 
       generate_global_interaction_matrix(args, train_arr, cv)
...
def generate_mask(labels,N, num_nodes):  
    num = 0
    n = num_nodes
...
def test_positive_sample(labels, test_arr):
    test_pos = np.zeros((len(test_arr),2))
    k = 0
    for i in range(len(test_arr)):
        test_pos[k, 0] = labels[i, 0]
        test_pos[k, 1] = labels[i, 1]
        k = k+1
    return test_pos    
...
def logging(msg, args):
  fpath = os.path.join( args.log_dir, "log.txt" )
