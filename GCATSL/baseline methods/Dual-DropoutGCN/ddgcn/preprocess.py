...
def data_loader(dataset, flag, not_computational):
    if dataset == 'SL':
        if flag==0:
        ...
        if not_computational:
          ...
          if flag==0:  
            ...
            with open("SynlethDB_extension/SL_Human_Approved.txt", "r") as inf:
                for line in inf:
                    name1, name2, _ = line.rstrip().split()
                    if name1 in id_mapping and name2 in id_mapping:  
                       inter_pairs.append((id_mapping[name1], id_mapping[name2]))
...
def split_graph(kfold, pairs, num_node, seed, flag):
    interaction = np.zeros((num_node, num_node))  
    interaction[pairs] = 1   
    ...
    c_set = set(zip(x, y)) - set(zip(x_train, y_train)) - set(zip(y_train, x_train))
    ...
    if kfold is not None:
        ...
        for train_indices, test_indices in kf.split(pairs):
            ...
            graph_mask = np.zeros((num_node, num_node))  
            
            neg_train, neg_test = next(neg_edge_kf)
            
            graph_mask[pairs[train_indices]] = 1
            graph_mask[noninter_pairs[neg_train[0:len(train_indices)]]] = 1
            ...
            graph_mask_kfold.append(graph_mask)

        return graph_train_kfold, graph_test_kfold, graph_mask_kfold
