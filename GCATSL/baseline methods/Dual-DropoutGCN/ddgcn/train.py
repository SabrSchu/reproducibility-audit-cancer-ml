...
T = 1
for t in range(T):
    inter_pairs, num_node = data_loader(DATASET, flag, not_computational=NOT_COMPUTATIONAL)
    ...
    graph_train_kfold, graph_test_kfold, graph_mask_kfold = split_graph(KFold, inter_pairs, num_node, SEED, flag)
    ...
    for i in range(KFold):
        ...
        mask = torch.FloatTensor(graph_mask)
        ...
        for j in range(EPOCH):
            ...
            loss = obj.cal_loss(reconstruct_adj_logit1, reconstruct_adj_logit2, mask, RHO)   #loss
            loss.backward()
            ...
            if (j % EVAL_INTER == 0) or need_early_stop_check or j+1 >= EPOCH:
                ...
                with torch.no_grad():
                    reconstruct_adj_logit1, reconstruct_adj_logit2 = model(feature1, feature2, adj_norm)
                    test_loss = obj_test.cal_loss(reconstruct_adj_logit1, reconstruct_adj_logit2, mask, RHO)
